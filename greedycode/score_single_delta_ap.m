function [rec, prec, ap] = score_single_delta_ap(c, g, n, delta)
...
for fr = min(c.fr):max(c.fr)
  ...
  while ~isempty(I) && ~isempty(J),
    ...
    [val, ind] = max(ov);
    if val > thr,
      pos(i) = g.r(J(ind(1))); % instead of just storing 1, store the actual index.
      ovval(i) = val ;
      J(ind) = [];
    end
    I(end) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% New code written by me %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
for i = min(r) : max(r)
  idx = find( r == i ) ;
  idxtmp = find( pos(idx) > 0 ) ;
  if isempty(idxtmp)
    continue ;
  end

  idxout = idx(idxtmp(1):end) ;


  % Don't consider tracks that are less than this length, so treat them as true positive anyways
  if (length(idx) < (delta+1)) || ((min(idxout)+delta) > length(pos))
    pos(idx) = pos(idx)>0 ;
  elseif pos(min(idxout)) == pos(min(idxout)+delta)
    % If the first and last frames correspond to the same box, then consider it as a true positive track.    
    pos(idx) = pos(idx)>0 ;
  else
    % If its greater than delta but not continuous, then rename the entire track as false positive.
    pos(idx) = 0 ;
  end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% End of code %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

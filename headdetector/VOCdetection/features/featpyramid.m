...
val = 2.0 ; % commented on 22nd october 2013 by Visesh Chari for CrowdMotions
% val = 3 ;
sc = val^(1/interval)
imsize = [size(im, 1) size(im, 2)];
max_scale = 15 + floor(log(min(imsize)/(5*sbin))/log(sc));
...
for i = 1:interval
  ...
  if extra_interval > 0
    ...
    pyra.feat{i} = features(scaled, sbin/(val^2));
    pyra.scales(i) = (val^2)/sc^(i-1);
  end
  % (sbin/2) x (sbin/2) features
  pyra.feat{i+extra_interval} = features(scaled, sbin/val);
  pyra.scales(i+extra_interval) = val/sc^(i-1);
  ...
  for j = i+interval:interval:max_scale
    scaled = resize(scaled, 1/val);
    pyra.feat{j+extra_interval+interval} = features(scaled, val*sbin);
    pyra.scales(j+extra_interval+interval) = 1/val * pyra.scales(j+extra_interval);
  end
end
disp('coming here');

...
%     load('Data/scene_data7') ;
    load('Data/scene_data7_4') ; param.ntrcks = 100 ;
...
%     xs(:,5) = 0.1 ;
    load('Data/scene_datashort7_4') ; param.ntrcks = 100 ; 
    load('Data/scene_gt7_2') ; 
%     gt = sub( gt, find( gt.fr < 56 ) ) ;
    gt = sub( gt, find( gt.fr < 57 ) ) ;
end
...
param.optimizationFn = @optimizeLPFlexible ; param.canonicalw = [1;0;1;0;1] ; 
...
% 														param.xs, param.frids, 4*pi/5, param.nvars + size( optstruct.Constraints{1}.A_data, 1 ), param.nvars, 0 ) ; optstruct.ConstraintNames{2} = 'TemporalConstraint2' ;                    
optstruct.nrelax = length(param.edge_velocity) ;% + length(param.edge_secvelocity) ;
...
model_yt.w = [1,0,1,0,1]'; % which weighting to use to get ytilde

...
col = round((rand(3,1e4))*255);  %% we assume number of tracks is less than 1e4.
...
for i = 1:len1
  ...
  if ~isempty(bbox)
    ...
    % im1 = show_bbox_on_image(im1, bbox(inds, :), bws, col, 4);
    im1 = show_bbox_on_image_dots(im1, bbox, bws, col, 4);
  end

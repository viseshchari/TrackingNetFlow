...
%LINESEGMENT[INTERSECT Intersections of line segments.
...
% Arrange output.
INT_B = triu(INT_B) ; % +triu(COINC_B) ;
[ei, ej] = ind2sub( size(INT_B), find(INT_B(:)>0) ) ;

% figure; hold on ;
% l1 = line( [XY1(ei, 1) XY1(ei, 3)]', [XY1(ei, 2) XY1(ei, 2)]' ) ;
% l2 = line( [XY2(1, ej); XY2(3, ej)], [XY2(2, ej); XY2(4, ej)] ) ;
% set( l1, 'color', 'r' ) ;
% set( l2, 'color', 'b' ) ;
% keyboard ;

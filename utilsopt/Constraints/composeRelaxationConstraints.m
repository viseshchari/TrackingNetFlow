function [A, b, edge_npairs, A_data] = composeRelaxationConstraints( edge_negpairs, nvars )
% function [A, b, edge_npairs] = composeRelaxationConstraints( edge_negpairs, nvars )
...
for i = 1 : edge_npairs
	A_data( 7*i-6, 1 ) = 3*i-2 ;
	A_data( 7*i-6, 2 ) = nvars+i ;
	A_data( 7*i-6, 3 ) = 1 ;

	A_data( 7*i-5, 1 ) = 3*i-2 ;
	A_data( 7*i-5, 2 ) = edge_negpairs(i, 1) ;
	A_data( 7*i-5, 3 ) = -1 ;

	A_data( 7*i-4, 1 ) = 3*i-1 ;
	A_data( 7*i-4, 2 ) = nvars+i ;
	A_data( 7*i-4, 3 ) = 1 ;

	A_data( 7*i-3, 1 ) = 3*i-1 ;
	A_data( 7*i-3, 2 ) = edge_negpairs(i, 2) ;
	A_data( 7*i-3, 3 ) = -1 ;

	A_data( 7*i-2, 1 ) = 3*i ;
	A_data( 7*i-2, 2 ) = edge_negpairs(i, 1) ;
	A_data( 7*i-2, 3 ) = 1 ;

	A_data( 7*i-1, 1 ) = 3*i ;
	A_data( 7*i-1, 2 ) = edge_negpairs(i, 2) ;
	A_data( 7*i-1, 3 ) = 1 ;

	A_data( 7*i, 1 ) = 3*i ;
	A_data( 7*i, 2 ) = nvars+i ;
	A_data( 7*i, 3 ) = -1 ;
end

A = sparse( A_data(:, 1), A_data(:, 2), A_data(:, 3), 3*edge_npairs, nvars+edge_npairs ) ;
b = zeros( 3*edge_npairs, 1  ) ;
b(3:3:end) = 1 ;